...
void memmove(void *dst, const void *src, uint32 n)
{
    char *d = dst;
    const char *s = src;
    if ((uint64)d > (uint64)s) { while (n) { --n; d[n] = s[n]; } }
    else { for (uint32 i = 0; i < n; ++i) d[i] = s[i]; }
}
...
static void printptr(uint32 fd, uint64 x)
{
    ...
    buf[i++] = '0';
    buf[i++] = 'x';
    for (j = 0; j < (int)(sizeof(uint64) * 2); j++, x <<= 4)
...
void print_fstat(file_stat_t *stat, char *filename)
{
    fprintf(STDOUT, "file %s: type = %s, inum = %d, nlink = %d, size = %d, offset = %d\n",
        filename, (stat->type < 3 ? file_type[stat->type] : "unknown"), stat->inode_num, (uint32)(stat->nlink), stat->size, stat->offset);
}
