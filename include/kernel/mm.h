...
/* 初始化物理内存分配器。
 * kernel_region: 内核专用的页面区域 (不会被用户态映射)
 * user_region:   用户态可用的页面区域 */
void pmem_init(void);
...
/* 页表根的类型由架构决定 (RISC-V 下是一个物理页号) */
typedef uint64 pgtbl_t;

/* 创建一个空页表, 返回其物理地址; 失败返回 0 */
pgtbl_t kvm_create(void);

/* 在页表中建立映射: va -> pa, 大小 size, 权限 perm。
 * 返回 0 成功, 负数失败。 */
...
/* 用户 -> 内核 拷贝。src_va 是用户地址。 */
int uvm_copyin(pgtbl_t pgtbl, uint64 dst_kva, uint64 src_va, uint64 len);

/* 从用户地址读取以 '\0' 结尾的字符串, 最多 max 字节。
 * 返回 0 成功, -1 表示地址非法或字符串过长。 */
int uvm_copyin_str(pgtbl_t pgtbl, char *dst, uint64 src_va, uint64 max);

/* 释放整个用户地址空间 */
void uvm_free(pgtbl_t pgtbl, uint64 sz);

/* 权限位 (与架构无关的抽象; arch 层负责翻译成实际的 PTE 位)。
 * 定义在 <kernel/perm.h> —— 因为 arch 层的 arch_perm_to_pte() 也要用它,
 * 而 arch 不应该反过来包含 generic 的内存管理头。 */
